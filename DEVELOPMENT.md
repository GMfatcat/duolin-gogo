...
- nineteenth slice implemented: `sql`, `http`, and `backend` now each have a deeper third wave that covers transactions and concurrency pitfalls, cache-validation and browser boundary mechanics, and practical reliability patterns such as idempotency keys, rate limiting, backpressure, dead-letter queues, and outbox publishing

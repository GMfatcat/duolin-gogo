...
- later authoring refinement: keep note-to-card scaffolding in `Library`, while `AI` stays focused on prompt and draft review so browsing and generation remain distinct
...
- AI draft review should allow both existing topic folders and user-created topic folder names, with new names normalized into safe local folder paths
